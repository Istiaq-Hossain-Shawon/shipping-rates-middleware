...
class ShippingRateMiddlewareApplicationTests {
	...
	// JUnit test for getting logistics Shiping Rate 
		@Test
		void getShippingRateTest() throws Exception {
			...
			MvcResult result = mockMvc.perform(post("/shipping-rates").principal(mockPrincipal).header(HttpHeaders.AUTHORIZATION, "Bearer "+token).content(JsonRequest).contentType(MediaType.APPLICATION_JSON_VALUE))
					.andExpect(status().isOk()).andReturn();
